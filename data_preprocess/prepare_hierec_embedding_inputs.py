...
def build_task_node_prompt(task: dict, node_row: dict) -> str:
    return (
        ...
        "write a task-specific research expertise profile for this taxonomy node. "
        "The text will be embedded and compared with experts' historical paper "
        "title/abstract embeddings, so write in a paper-like research expertise "
        "style, not as a job ad or staffing request. Use the paper title/abstract "
        "as the main source of context; use the node name and subtree skills only "
        "to locate the relevant aspect of the task.\n\n"
        ...
        "role_description, key_capabilities, evidence_from_abstract. The "
        "requirement and role_description should be the same concise research "
        "expertise paragraph. It should describe task-specific methods, concepts, "
        "datasets, systems, and evaluation capabilities under this node. Do not "
        "simply restate the taxonomy node name or subtree skills. Use the subtree "
        "skills to select only the relevant part of the paper title/abstract. Do "
        "not copy the whole paper contribution into every node. Include only the "
        "task-specific capability that this node is responsible for. If the "
        "subtree has one or a few skills, treat sibling methods and unrelated "
        "abstract details as out of scope; mention them only when they are needed "
        "as brief task context, not as required capabilities. Avoid phrases like "
        "'This task requires an expert who can'."
    )
